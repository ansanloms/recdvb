...
struct {
	...
	{ "101", "bs15_1", 0x40f1 }, { "103", "bs03_1", 0x4031 },
	...
	{ "202", "bs15_2", 0x40f2 }, { "211", "bs09_0", 0x4490 },
	...
	{ "910", "bs15_2", 0x40f2 }, { "929", "bs15_1", 0x40f1 },
...
void set_lch(char *lch, char **ppch, char **sid, unsigned int *tsid)
{
	...
	for (i = 0; i < NUM_PRESET_CH; i++)
		if (strcmp(preset_ch[i].lch, lch) == 0) break;
	if (i < NUM_PRESET_CH ) {
		*ppch = preset_ch[i].channel;
		if (*tsid == 0) *tsid = preset_ch[i].tsid;
		if (*sid == NULL) *sid = preset_ch[i].lch;
	}
}

void set_pch(char *pch, char **sid, unsigned int *tsid)
{
	int i;
	for (i = 0; i < NUM_PRESET_CH; i++)
		if( (strncmp(preset_ch[i].channel, "bs", 2) == 0) && (strcmp(preset_ch[i].channel+2, pch+2) == 0) ) break;
...
if(use_http){	// http-server add-
    ...
    if(pch == NULL){
        pch = argv[optind];
        if(strlen(pch) == 6){
            if( ((pch[0]=='b')||(pch[0]=='B')) && ((pch[1]=='s')||(pch[1]=='S')) && (pch[4]=='_')){
                set_pch(pch, &sid_list, &tsid);
                if(sid_list) use_splitter = TRUE;
                fprintf(stderr, "tsid = 0x%x\n", tsid);
            }
        }
